        ...
        <section className={styles.graphCard}>
          ...
          <div className={styles.chart}>
            ...
            <div className={styles.bars}>
              {EMOTION_RESULTS.map(({ id, label, characterImg, iconImg, barClassName }) => {
                const count = counts[id]
                const barHeight = Math.max((count / maxCount) * 58, count > 0 ? 18 : 7)
                const characterOffset = count === 0 ? -1.6 : -0.8
                const countOffset = count === 0 ? 7.6 : 9
                const characterStyle = { bottom: `calc(${barHeight}% + ${characterOffset}rem)` }
                const countStyle = { bottom: `calc(${barHeight}% + ${countOffset}rem)` }

                return (
                  <div key={id} className={styles.barGroup}>
                    <div className={styles.barArea}>
                      <div className={styles.countBubble} style={countStyle}>
                        <span className={styles.countBubbleValue}>{count}</span>
                        <span className={styles.countBubbleUnit}>개</span>
                      </div>
                      <div className={styles.barCharacterBox} style={characterStyle}>
